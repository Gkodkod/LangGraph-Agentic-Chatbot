...
async def answer_grader(question: str, answer: str) -> GradeAnswer:
    ...
    # llm = ChatOpenAI(api_key=api_key, model="gpt-oss-120b", temperature=0)
    # llm = ChatOpenAI(api_key=api_key, model="gemini-2.5-flash", temperature=0)
    llm = ChatGroq(
    ...
    return grade.binary_score.strip().lower() == "yes"
...
async def generate_answer(context_doc: list[Document], question: str) -> str:
    ...
    generated_text = await generation_chain.ainvoke(
...
async def hallucination_grader(context_docs: list[Document], generation: str) -> bool:
    ...
    grade = await hallucination_grader.ainvoke(
        {"context_docs": context_docs, "generation": generation}
    )
    return grade.binary_score.strip().lower() == "yes"
...
async def rephrase_question(question: str, chat_history: list[dict[str, str]]) -> str:
    ...
    rephrased_question = await rephrase_chain.ainvoke(
        {"chat_history": chat_history_str, "input": question}
    )
